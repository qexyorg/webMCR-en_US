...
$save_conf_err = 'Error create \ replace file '.MCR_ROOT.'config.php (root directory of site). No permissions for change file (chmod).';

$i_sd = 'other/install/';

$content_advice = 'Fill the form for complete installation '.PROGNAME;
...
if ( !extension_loaded('gd') ) 
	...
	$cErr  .= $p.'Library GD not exist'.$pe;

if ( ini_get('register_globals')  ) 
	
	$cErr .= $p.'File php.ini settings of PHP [Option] <b>register_globals = On</b>. Change to <b>Off</b>'.$pe;

if ( !function_exists('fsockopen')) 
	
	$cErr .= $p.'Function fsockopen not exist'.$pe;

if ( !function_exists('json_encode')) 
	
	$cErr .= $p.'Function json_encode not exist'.$pe;
...
function checkRWOut($fname, $create = false)
{
    ...
    if (!checkRW($fname, $create))
        $cErr .= '<p>' . ($is_dir ? 'File' : 'Folder') . ' ' . $fname . ' . You don\'t have permissions for change </p>';
}
...
function findCMS($way)
{
    ...
    if (!TextBase::StringLen($way)) {
        $info = 'Set path to folder ' . $main_cms . '.';
        return false;
    }
    ...
    if (!file_exists($way . $file))
        $info = 'Path to folder ' . $main_cms . ' is incorrect. File ' . $file . ' not exist in this folder';
    else
        return true;
...
switch ($step) {
	...
	case 1:     
            ...
            if (!$mysql_port)
                $info = 'Specify the port to connect to the database.';
            elseif (!TextBase::StringLen($mysql_adress))
                $info = 'Specify the address of the server MySQL.';
            elseif (!TextBase::StringLen($mysql_user))
                $info = 'Specify a username to connect to the MySQL server.';
            else {
                ...
                if ($connect_result !== true)
                    $info = 'Error connection to database: ' . $connect_result;
                else {
	...
	case 2:
            ...
            if (!TextBase::StringLen($site_user)) {
                ...
                $info = 'Specify the username.';
                break;
            }

            $connect_result = DBinit();
            if ($connect_result !== true) {
                $info = 'Error connection settings with database.';
                break;
            }
            ...
            if ($main_cms) {
                ...
                if (!TextBase::StringLen($bd_names['users'])) {
                    $info = 'Enter the name of the user table.';
                    break;
                }
                ...
                if ($userId === false) {
                    $info = 'The name of the users table is incorrect.';
                    break;
                } elseif (!$userId) {
                    $info = 'This user name is not found.';
                    break;
                }
                ...
                if ($mode == 'xenforo') {
                    ...
                    if ($result === false) {
                        $info = 'Name of the table with additional data invalid.';
                        break;
                    }
                }
	...
	case 3:
            ...
            if (TextBase::StringLen($keywords) > 200)
                $info = 'Keywords occupy more than 200 characters (' . TextBase::StringLen($keywords) . ').';
            elseif (!$timezone)
                $info = 'Select the time zone.';
            else {
