...
class Profile extends View
{
    ...
    public function Show($modal_mode = true)
    {
        ...
        $main_info['active_last'] = ($tmpParam) ? self::TimeFrom($tmpParam) : 'Never'; // toDo show Online \ Offline	
        ...
        $user_info['play_times'] = array((int) $statistic['play_times'], 'Games played');

        $tmpParam = $this->user->gameLoginLast();
        $user_info['play_last'] = array(($tmpParam) ? self::TimeFrom($tmpParam) : 'Never', 'Login to the game');

        $tmpParam = $this->user->getStatisticTime('create_time');

        $user_info['create_time'] = array(($tmpParam) ? $tmpParam : 'Unknown', 'Date of registration');

        $user_info['comments_num'] = array((int) $statistic['comments_num'], 'Comments');
        $user_info['undress_times'] = array((int) $statistic['undress_times'], 'Reincarnation');
